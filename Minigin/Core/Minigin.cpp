...
{
	static_cast<minigin::Minigin*>(arg)->RunOneFrame();
}
#endif
...
minigin::Minigin::Minigin(const std::filesystem::path& dataPath, const std::string& title, int windowWidth, int windowHeight)
{
	PrintSDLVersion();
	...
	(
		title.c_str(),		// sometimes a leak happens here!
		windowWidth,
		windowHeight,
		SDL_WINDOW_OPENGL
	);
